...
int main(void)
{
	...
	uint8_t temp;
	uint8_t tempPrev;
	uint16_t temp_sum=0; //variable auxiliary
	uint8_t tempTab[10]; //table for temperature
	uint8_t temp_average=0;
	int i=0;
	int j=0;
	...
	while (1) {
		...
		default:
			...
			if(j==0) //condition to loop performed only once
			{
			for(i; i<10; i++)
			{
			readI2CMaster(I2C_ADDR_7BIT_1, &ledState, &temp);
			tempTab[i]=temp;
			temp_sum=temp_sum+tempTab[i];
			DEBUGOUT("Temp: %d\n", tempTab[i]);
			DEBUGOUT("Temp_sum%d", i);
			DEBUGOUT(": %d\n", temp_sum);
			_delay_ms(1000); //wait for 1sec.
			}

			DEBUGOUT("Temp_sumAll: %d\n", temp_sum);
			temp_average = temp_sum/10;
			DEBUGOUT("Temp_average: %d\n", temp_average);
			temp=temp_average;
			j=1;
			}
